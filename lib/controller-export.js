...
class ControllerExport {
	...
	async uploadToS3(partNumber, file) {
		...
		const filename = `exports/${this.session.clientCode}/${this.userId}/${this.entity}-part${partNumber}.xlsx`;
	...
	getFilesPaths(filenames) {
		...
		return Promise.all(filenames.map(filename => S3.getSignedUrl('getObject', {
			Bucket: bucket,
			Key: filename,
			Expires: this.fileUrlExpirationTime // Configurable, con un default de 1 día (en segundos)
		})));
	}
}
